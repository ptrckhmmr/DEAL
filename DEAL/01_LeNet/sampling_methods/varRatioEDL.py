...
"""Variation Ratio based AL method for DEAL.

Samples in batches based on variation ratio.
"""
...
class VarRatioEDL(SamplingMethod):
  def __init__(self, X, y, seed):
    self.X = X
    self.y = y
    self.name = 'VarRatioEDL'

    self.dropout_iterations = 1
  ...
  def select_batch_(self, model, already_selected, N, **kwargs):
    ...
    for d in range(self.dropout_iterations):
      ...
      try:
        distances, pred, features = model.decision_function(self.X, self.test_time_dropout)
      except:
