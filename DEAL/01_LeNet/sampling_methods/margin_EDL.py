...
"""Margin based AL method for DEAL.
...
class MarginEDL(SamplingMethod):
  ...
  def select_batch_(self, model, already_selected, N, **kwargs):
    """Returns batch of datapoints with smallest margin for DEAL.
    ...
    try:
      distances, prediction, features = model.decision_function(self.X, self.test_time_dropout)
    except:
    ...
    if len(prediction.shape) < 2:
      min_margin = abs(prediction)
    else:
      sort_distances = np.sort(prediction, 1)[:, -2:]
