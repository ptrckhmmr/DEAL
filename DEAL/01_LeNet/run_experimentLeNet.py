...
flags.DEFINE_float(
    "warmstart_size", 100, #mnist: 0.00173 oder 0.00179 ; CIFAR-10: 1./24 # Data Split Umstellen
    ("Can be float or integer.  Float indicates percentage of training data "
     "to use in the initial warmstart model")
)
flags.DEFINE_float(
    "batch_size", 100, #mnist: 0.00173 oder 0.00179 ; CIFAR-10: 1./24 # Data Split Umstellen
...
flags.DEFINE_string(
    "score_method", "LeNet_DEAL",
    "Method to use to calculate accuracy.")
flags.DEFINE_string(
    "select_method", "LeNet_DEAL",
...
flags.DEFINE_float("train_horizon", "1", #mnist: 0.0344 ; CIFAR-10: 0.4166666
  ...
  for c in confusions:
    for m in mixtures:
      for seed in range(starting_seed, starting_seed + FLAGS.trials):
        ...
        with open('./test_accuracy/accuracyLeNet_' + str(seed) + '.json', 'w') as f:
