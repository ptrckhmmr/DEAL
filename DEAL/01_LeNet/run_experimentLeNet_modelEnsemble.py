...
Supported datasets include mnist, letter, cifar10.
...
flags.DEFINE_string(
    "score_method", "LeNet_Softmax",
    "Method to use to calculate accuracy.")
flags.DEFINE_string(
    "select_method", "LeNet_Softmax",
...
flags.DEFINE_float("train_horizon", "1", #CIFAR-10: 0.4166666 MNIST: 0.0344 # ALL: 1 !!! DataSplit umstellen
...
def generate_one_curve(X,
  ...
  for b in range(n_batches):
    ...
    for i in range(n_ensembles):
        ...
        # Enable/disable test time dropout
        test_time_dropout = False

        # Make predictions at test time
        try:
            pred = score_model.predict(X_Pool_Dropout, test_time_dropout)
        except:
  ...
  for c in confusions:
    for m in mixtures:
      for seed in range(starting_seed, starting_seed + FLAGS.trials):
        ...
        print('CONFUSIONS: ', len(confusions))
        ...
        results, sampler_state, accuracy, sampling_time_measurement = generate_one_curve(
