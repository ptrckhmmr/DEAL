...
"""Utility functions for all run_experiment.py. files."""
...
from utils.ResNet_DEAL import ResNet_DEAL
from utils.ResNet_Softmax import ResNet_Softmax
...
def get_mldata(data_dir, name):
  ...
  if dataname == "checkerboard":
    ...
    filename_X = os.path.join(data_dir, "X_audi_exterior_256" + ".pkl")
    filename_y = os.path.join(data_dir, "y_audi_exterior_256" + ".pkl")

    with open(filename_X, 'rb') as f:
      X_list_train_val = pickle.load(f)
    ...
    X = []
    for i in range(len(X_list_train_val)):
      image_ind = i
    ...
    X_train_val, y_train_val = shuffle(X_train_val, y_train_val, random_state=49)

    X = X_train_val
    y = y_train_val


  elif dataname == "medical":
    filename_X = os.path.join(data_dir, "X_pneu_128" + ".pkl")
    filename_y = os.path.join(data_dir, "y_pneu_128" + ".pkl")
    ...
    X_train_val = X_train_val[:-52]
    y_train_val = y_train_val[:-52]
    ...
    dataname_train = "train_32x32"
    dataname_test = "test_32x32"
    filename_train = os.path.join(data_dir, dataname_train + ".mat")
    print(filename_train)
    filename_test = os.path.join(data_dir, dataname_test + ".mat")
    print(filename_test)

    train_data = sio.loadmat(filename_train)
    test_data = sio.loadmat(filename_test)
    ...
    X_data = np.concatenate([x_train, x_test], axis=3)
    ...
    for i in range(X_data.shape[3]):
      image_ind = i
      X.append(rgb2gray(X_data[:, :, :, image_ind]))
    X = np.array(X)
    X = X[:, :, :, np.newaxis]
    print('Shape von SVHN')
    print(X.shape)
...
def flip_label(y, percent_random):
  ...
  classes = np.unique(y)
  y_orig = copy.copy(y)
  print(y_orig.shape[0])
  indices = np.arange(y_orig.shape[0])
...
def parse_args():
    ...
    parser.add_argument('--network', type=str, default='ResNet_Softmax', help='ResNet_Softmax or ResNet_DEAL?')
    parser.add_argument('--phase', type=str, default='train', help='train or test ?')
    parser.add_argument('--dataset', type=str, default='tiny', help='[cifar10, cifar100, mnist, fashion-mnist, tiny, svhn')
...
def get_model(method, seed=13):
  ...
  elif method == "ResNet_DEAL":
    # open session
    #with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True))
    model = ResNet_DEAL(sess, args)
  elif method == "ResNet_Softmax":
    # open session
    #with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True))
    model = ResNet_Softmax(sess, args)
  else:
